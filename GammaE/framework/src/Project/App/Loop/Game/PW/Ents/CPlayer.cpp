...
// -----------------------------------------------------------------------------
#define SQ(a) ((a)*(a))
void CPlayer::Init(MTH_CPoint3& _oPos,int _iPlayerID)
{
	// Set the player ID
...
// -----------------------------------------------------------------------------
MTH_CPoint3 oGetIPTDir(IPT_Stick& _oIPT)
{
	MTH_CPoint3 oDir;
...
// -----------------------------------------------------------------------------
MTH_CPoint3 gMAXSPD		(200.0f,200.0f,1e-4f);
...
void CPlayer::CheckNewPos(float _fDeltaT)
{
	...
	MTH_CPoint3 oLDir	= oGetIPTDir(*poLStick);
...
void CPlayer::Think(float _fDeltaT)
{
	...
	switch (m_eState)
	{
		case ENTSTATE_ALIVE:
		{
			...
			MTH_CPoint3 oPos(m_fTCOfs,m_fHeight,0.0f);
			*/
			...
			if (gaInput[0].pressed.cross)
			{
				...
				{
					MTH_CPoint3 oFPos1 = m_poGI->poGraphicResource()->oGetCP("CP_Fire1");
					MTH_CPoint3 oFPos2 = m_poGI->poGraphicResource()->oGetCP("CP_Fire2");
					
					MTH_CMat4x4& oMat = m_poGI->roTransform();
					
					MTH_CPoint3 oWFPos1,oWFPos2;
					roMat.MultVec3(oFPos1,oWFPos1);
					roMat.MultVec3(oFPos2,oWFPos2);

					m_oWD[0].m_fShotTime += m_oWD[0].m_fMaxShotTime;
					CBulletMgr::iAddBullet(0,oWFPos1,MTH_CPoint3::oX(),this);
					CBulletMgr::iAddBullet(0,oWFPos2,MTH_CPoint3::oX(),this);
				}
			}
			...
			if (gaInput[0].pressed.r1)
			{
				...
				{
					MTH_CPoint3 oFPos0 = m_poGI->poGraphicResource()->oGetCP("CP_Fire1");
					MTH_CPoint3 oFPos1 = m_poGI->poGraphicResource()->oGetCP("CP_Fire2");
					MTH_CPoint3 oFPos2 = m_poGI->poGraphicResource()->oGetCP("CP_Laser");

					MTH_CMat4x4& oMat = m_poGI->roTransform();

					MTH_CPoint3 oWFPos0,oWFPos1,oWFPos2;
					roMat.MultVec3(oFPos0,oWFPos0);
					roMat.MultVec3(oFPos1,oWFPos1);
					roMat.MultVec3(oFPos2,oWFPos2);

					m_oWD[1].m_fShotTime += m_oWD[1].m_fMaxShotTime;
					CBulletMgr::iAddBullet(1,oWFPos2,MTH_CPoint3::oX(),this);
				}
			}
			...
			if (gaInput[0].pressed.r2)
			{
				...
				if (m_oWD[2].m_fShotTime <= 0.0f)
				{
					...
					CBulletMgr::iAddBullet(2,m_oPos,MTH_CPoint3::oX(),this);
				}
				*/
...
{
	CFlareMgr::iAddFlare((int)(fRand()*3.0f),MTH_CPoint3(0,0,0),1.0f);
}
// -----------------------------------------------------------------------------
*/
#ifdef PROFILE
