...
#include "CHUD.h"
// ----------------------------------------------------------------------------
CHUDElem::CHUDElem(CObject3D *_poObj, float _x, float _y, float _tx, float _ty)
{
...
void CHUDElem::Setup(CObject3D *_poObj, float _x, float _y, float _tx, float _ty)
{	
	x  = _x;
	y  = _y;
	tx = _tx;
...
// ----------------------------------------------------------------------------
uint CHUD::uiAddObject(CObject3D *_poObj, float _x, float _y, float _tx, float _ty,const CGString& _sName)
{
		...
		for (uint i=0;i<m_oHUDObjs.uiNumElems();i++)
		{
			if ( m_oHUDObjs[i]->poObj )
			{
				...
				// CGRenderer::I()->PushCameraMatrix();
					...
					BVol = m_oHUDObjs[i]->poObj->poGetBoundVol();
