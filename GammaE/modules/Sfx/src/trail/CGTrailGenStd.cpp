...
void CGTrailGenStd::InitTrail(CGTrail* _poTrail)
{
	...
	// Set initial energy
	poTrail->m_fIniEnergy = m_fIniEnergy*(1.0f + MATH_Common::fSRand()*m_fRndEnergy);
	float f1OverEnergy = 1.0f / poTrail->m_fIniEnergy;

	// Initialize Size
	poTrail->m_fIniSize = m_fIniSize*(1.0f + MATH_Common::fSRand()*m_fIniRndSize); 
	poTrail->m_fEndSize = m_fEndSize*(1.0f + MATH_Common::fSRand()*m_fEndRndSize);
...
void CGTrailGenStd::UpdateInstance(CGTrailInstance* _poInst,float _fDeltaT)
{
	...
	while ((bLastBB == false) && (uiNumBBs < uiNumSubdivisions))
	{
		...
		// Size
		oU1.Scale( MATH_Common::fLerp(poTrail->m_fIniSize,poTrail->m_fEndSize,fFact) );
		...
		{
			// Get first point
			oVX[0] = oP0; oVX[0].Sub(oU1);
			oVX[1] = oP0; oVX[1].Add(oU1);
		...
		// Build geometry
		oVX[2] = oP1; oVX[2].Add(oU1);
		oVX[3] = oP1; oVX[3].Sub(oU1);
