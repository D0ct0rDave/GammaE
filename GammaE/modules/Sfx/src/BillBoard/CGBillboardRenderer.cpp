...
// ----------------------------------------------------------------------------
void CGBillboardRenderer::AddBillboards(CVect3* _poVX,CVect2* _poUV,CGColor* _poVC,uint _uiNumBillboards)
{
	/// WARNING: Sanity checks here!!!
...
void CGBillboardRenderer::Render()
{
	...
	CGRenderer::I()->ClearWorldMatrix();
		...
		// Build the meshes
		CVect3* poVX = m_oGeoPool.VXs;
		CVect2* poUV = m_oGeoPool.UVs;
		CGColor* poVC = m_oGeoPool.VCs;

		for (uint j=0;j<m_oBBRL.uiNumElems();j++)
		{
			CMesh& oMesh = m_oMeshes.oGet();

			oMesh.eMeshType = m_oGeoPool.eMeshType;
			...
			if (uiNumBBs>0)
			{
				oMesh.usNumPrims = uiNumBBs;
				oMesh.usNumVerts = uiNumBBs*4;
