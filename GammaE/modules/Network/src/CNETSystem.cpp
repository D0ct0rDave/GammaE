...
typedef struct{
	...
	unsigned int		uiRcvTimeStamp;	//-----------------------------------------------------------------------------
	
	CNETTimeStampList	oTSL;			//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void InitClientData(TClientData* _poData,unsigned int _uiPort,unsigned int _uiAddress,int _iSocket)
{	
	_poData->bValid					= true;
...
//-----------------------------------------------------------------------------
inline bool INTPendingInfo(unsigned int _iSocket,SOCKADDR_IN *_pInAddr,void *_pBuf,int _iSize)
{
	int		iRes;
	char	cBuffer[MAX_PACKET_SIZE+8];
	TNETPacket* poPacket = (TNETPacket*)cBuffer;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNETSystem::bPendingInfo(int _iSocket)
{
	SOCKADDR_IN oInAddr;
...
//-----------------------------------------------------------------------------
int CNETSystem::WriteToServer (void *_pBuff,int _iSize)
{
...
//-----------------------------------------------------------------------------
int CNETSystem::ReadFromServer(void *_pBuff,int _iSize)
{
	Block(goCD[CLIENT_SOCKET].iSocket);
