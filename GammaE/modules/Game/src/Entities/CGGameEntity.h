...
public:
		...
		CGString sUID() { return( sClass() + CGString('.') + CGString((int)m_uiID) ); };
		
		/// A numeric class identifier
		uint    uiClass();

		/// The numeric identifier for this entity
		uint    uiID() { return(m_uiID); };
		...
		TEntityState eGetState() { return(m_eState); };
...
public:
		...
		bool bIsEnabled() { return(m_bEnabled); };

		/// Return the life time multiplier for this entity
		float fTimeMult() { return( m_fTimeMult); };

		/// Changes the life time multiplier for this entity
		void TimeMult(float _fTimeMult) { m_fTimeMult = _fTimeMult; };

		/// Returns the entity time
		float fTime() { return(m_fTime); };

		/// Sets the entity time
		void Time(float _fTime) { m_fTime = _fTime; };
		
		/// Retrieves the time delta for this entity
		float fDeltaT() { return(m_fDeltaT); };
		

// Compound of ... (vs inheritance approach)

		/// Sets the graphic instance for the entity
		void GraphicInstance(CGGraphicInstance* _poVis) { m_poGraph = _poVis; };

		/// Gets the graphic instance of the entity
		CGGraphicInstance* poGraphicInstance() { return(m_poGraph); };

		/// Sets the collision information for the entity
		void Collider(CCOL_Collider* _poCol) { m_poCol = _poCol; };

		/// Gets the collision information of the entity
		CCOL_Collider* poCollider() { return(m_poCol); };

		/// Sets the resource that handles the logic of this entity
		void Logic(CGScriptInstance* _poLogic) { m_poLogic = _poLogic; };

		/// Gets the resource that handles the logic of this entity
		CGScriptInstance* poLogic() { return(m_poLogic); };

		/// Sets the input handler for the entity
		void Input(CGInputHandler* _poInput) { m_poInput = _poInput; };

		/// Retrieves the input handler of the entity
		CGInputHandler* poInput() { return (m_poInput); };

		/// Sets the logger message channel for the entity
		void Logger(CGLogger* _poLogger) { m_poLogger = _poLogger; };

		/// Retrieves the logger message channel for the entity
		CGLogger* poLogger() { return (m_poLogger); };

		/// Adds a user data object to the entity
		uint uiAddUserData(uobject _uoData) { return( m_uoUserData.uiAdd(_uoData) ); };
		...
		void ClearUserData() { m_uoUserData.Clear(); };
