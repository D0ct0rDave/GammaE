...
HWND		mainhWnd;
bool		bApplicationCreated = false;
...
{
	FILE *fd;
	fd = fopen("files.txt","wt");
	if (fd)
	{
		for(int i=0;i<oFileList.iNumFiles;i++)
			if (oFileList.FN[i][0])
				fprintf(fd,"%s\n",oFileList.FN[i]);
...
LONG WINAPI WinProcInstance(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	  switch (message)
	  {
			...
			case WM_KEYDOWN:CApplication::OnButtonDown(wParam & 0x000000ff);	
							break;

			case WM_KEYUP:	CApplication::OnButtonUp(wParam & 0x000000ff);
							break;

			case WM_MOUSEMOVE:	CApplication::OnCursorMove(	LOWORD(lParam), 
															HIWORD(lParam),
															wParam & (MK_LBUTTON | MK_MBUTTON | MK_RBUTTON) );
							break;

			case WM_LBUTTONUP:	CApplication::OnButtonUp(0/*LOWORD(lParam), HIWORD(lParam),0*/); break;
			case WM_RBUTTONUP:	CApplication::OnButtonUp(1/*LOWORD(lParam), HIWORD(lParam),1*/); break;
			case WM_MBUTTONUP:	CApplication::OnButtonUp(2/*LOWORD(lParam), HIWORD(lParam),2*/); break;

			case WM_LBUTTONDOWN:CApplication::OnButtonDown(0/*LOWORD(lParam), HIWORD(lParam),0*/); break;
			case WM_RBUTTONDOWN:CApplication::OnButtonDown(1/*LOWORD(lParam), HIWORD(lParam),1*/); break;
			case WM_MBUTTONDOWN:CApplication::OnButtonDown(2/*LOWORD(lParam), HIWORD(lParam),2*/); break;
		/*
...
void WindowSystemInit( HINSTANCE hInstance )
{
	...
	{
		unsigned int WND_STYLE = WS_POPUP | WS_SYSMENU | WS_BORDER | WS_CAPTION | WS_VISIBLE;
		...
		::AdjustWindowRect(&rClient, WND_STYLE, false);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	while ( ! bFinnishApp )
	{
		if (PeekMessage(&msg, mainhWnd, 0, 0, PM_REMOVE))
		{
