...
//-----------------------------------------------------------------------------
void CEnemyGroupExt::Init(unsigned int _uiNumElems,float _fSpawnTime,float _fEnergy,const CVect3& _oPos,const CGString& _sAIScript,const CGString& _sGraphicRes,const CGString& _sPath,float _fScale,uint _uiIniColor,uint _uiEndColor)
{
	// Set the spanw time
	...
	for (uint i=0;i<_uiNumElems;i++)
	{
		...
		float fScale = 1.0f + (MATH_Common::fSRand() * _fScale);
		...
		// Set entity color
		CGColor oColor;
		oColor.Lerp(oIniColor,oEndColor,MATH_Common::fRand());
